...
void ClosedCube_HDC1080::begin(uint8_t address, SoftwareWire bus) {
	_address = address;
	_bus = bus;
	_bus.begin();
...
void ClosedCube_HDC1080::writeRegister(HDC1080_Registers reg) {
	...
	_bus.write(HDC1080_CONFIGURATION);
	_bus.write(reg.rawData);
	_bus.write((byte) 0x00);
...
void ClosedCube_HDC1080::heatUp(uint8_t seconds) {
	...
	for (int i = 1; i < (seconds*66); i++) {
		...
		_bus.endTransmission();
		delay(20);
		_bus.requestFrom(_address, (uint8_t)4);
		_bus.readBytes(buf, (size_t)4);
	}
	reg.Heater = 0;
