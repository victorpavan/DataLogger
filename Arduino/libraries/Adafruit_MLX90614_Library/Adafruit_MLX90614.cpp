 ...
 */
bool Adafruit_MLX90614::begin(SoftwareWire bus) {
  _bus = bus;
  _bus.begin();
...
uint16_t Adafruit_MLX90614::read16(uint8_t a) {
  ...
  _bus.beginTransmission(_addr); // start transmission to device
  _bus.write(a);                 // sends register address to read from
  _bus.endTransmission(false);   // end transmission

  _bus.requestFrom(_addr, (size_t)3); // send data n-bytes read
  ret = _bus.read();                  // receive DATA
  ret |= _bus.read() << 8;            // receive DATA

  uint8_t pec = _bus.read();
...
void Adafruit_MLX90614::write16(uint8_t a, uint16_t v) {
  ...
  _bus.beginTransmission(_addr); // start transmission to device
  _bus.write(a);                 // sends register address to write
  _bus.write(v & 0xff);          // lo
  _bus.write(v >> 8);            // hi
  _bus.write(pec);               // pec
  _bus.endTransmission(true);    // end transmission
}
